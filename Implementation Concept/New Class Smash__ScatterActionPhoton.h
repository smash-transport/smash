...
class ScatterActionPhoton : public ScatterAction {
  public:
    ...
    void generate_final_state() override; // modify so only photon producing actions can take place (still virtually of course) and weigh them correctly
    float raw_weight_value() const override { return weight_; } // has to be made virtual
...
/*
TO DO: overwrite all necessary functions of ScatterAction (should be only generate_final_state() )
*/