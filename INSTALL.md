...
SMASH is tightly coupled to Pythia and thus requires a specific version, which is currently `8.312`.
...
tar xf pythia8312.tgz && rm pythia8312.tgz
cd pythia8312
...
```console
curl https://pythia.org/download/pythia83/pythia8312.tgz -o pythia8312.tgz
tar xf pythia8312.tgz && rm pythia8312.tgz
cd pythia8312
...
*  Either set the bash environment variables `PYTHIA8` or `PYTHIA_ROOT_DIR` (e.g. `export PYTHIA_ROOT_DIR=/opt/pythia8312`) or
*  use the CMake `-DPYTHIA_ROOT_DIR` option (e.g. `cmake -DPYTHIA_ROOT_DIR=/opt/pythia8312 ..`).
