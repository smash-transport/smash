...
SMASH is tightly coupled to Pythia and thus requires a specific version, which is currently `8.311`.
...
tar xf pythia8311.tgz && rm pythia8311.tgz
cd pythia8311
...
```console
curl https://pythia.org/download/pythia83/pythia8311.tgz -o pythia8311.tgz
tar xf pythia8311.tgz && rm pythia8311.tgz
cd pythia8311
...
mkdir build
cd build
cmake -DPythia_CONFIG_EXECUTABLE=[...]/pythia8311/bin/pythia8-config ..
make
```
...
*  Either set the bash environment variables `PYTHIA8` or `PYTHIA_ROOT_DIR` (e.g. `export PYTHIA_ROOT_DIR=/opt/pythia8311`) or
*  use the CMake `-DPYTHIA_ROOT_DIR` option (e.g. `cmake -DPYTHIA_ROOT_DIR=/opt/pythia8311 ..`).
