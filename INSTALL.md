...
SMASH is tightly coupled to Pythia and thus requires a specific version, which is currently `8.317`.
...
tar xf pythia8317.tgz && rm pythia8317.tgz
cd pythia8317
...
```console
curl https://pythia.org/download/pythia83/pythia8317.tgz -o pythia8317.tgz
tar xf pythia8317.tgz && rm pythia8317.tgz
cd pythia8317
...
mkdir build
cd build
cmake -DPythia_CONFIG_EXECUTABLE=[...]/pythia8317/bin/pythia8-config ..
make
```
...
*  Either set the bash environment variables `PYTHIA8` or `PYTHIA_ROOT_DIR` (e.g. `export PYTHIA_ROOT_DIR=/opt/pythia8317`) or
*  use the CMake `-DPYTHIA_ROOT_DIR` option (e.g. `cmake -DPYTHIA_ROOT_DIR=/opt/pythia8317 ..`).
