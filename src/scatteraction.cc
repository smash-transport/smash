...
void ScatterAction::add_all_processes(float elastic_parameter,
                                      ...
                                      double low_snn_cut,
                                      bool strings_switch) {
  /* The string fragmentation is implemented in the same way in GiBUU (Physics Reports
   * 512(2012), 1-124, pg. 33). If the center of mass energy is low, two particles scatter
   * through the resonance channels. If high, the out going particles are generated by string
   * fragmentation. If in between, the out going particles are generated either through the
   * resonance channels or string fragmentation by chance. In detail, the low energy regoin is
   * from the threshold to (mix_scatter_type_energy - mix_scatter_type_window_width), while the
   * high energy region is from (mix_scatter_type_energy + mix_scatter_type_window_width) to
   * infinity. In between, the probability for string fragmentation increases linearly from 0
   * to 1 as the c.m. energy.*/
  // Determine the energy region of the mixed scattering type for two types of scattering.
  ...
  // nucleon-nucleon collisions are included in pythia.
      include_pythia = true;
  } else if ((incoming_particles_[0].type().pdgcode().is_pion() &&
              incoming_particles_[1].type().is_nucleon()) ||
  ...
  // string fragmentation is enabled when strings_switch is on and the process is included in pythia.
  const bool enable_pythia = strings_switch && include_pythia;
  bool is_pythia = false; // Whether the scattering is through string fragmentaion
  if (enable_pythia) {
     if (sqrt_s() > mix_scatter_type_energy + mix_scatter_type_window_width) {
        is_pythia = true; // scatterings at high energies are through string fragmentation
     } else if (sqrt_s() > mix_scatter_type_energy - mix_scatter_type_window_width) {
        const float probability_pythia = (sqrt_s() - mix_scatter_type_energy +
                    mix_scatter_type_window_width) / mix_scatter_type_window_width / 2.0;
        if (probability_pythia > Random::uniform(0.f, 1.f)) {
           // scatterings at the middle energies are through string fragmentation by chance.
           is_pythia = true; 
        }
     }
  }
 // The parametrization of the elastic scattering is currently universal from the low to high energy.
       if (two_to_two && (!incoming_particles_[0].type().is_nucleon() ||
           !incoming_particles_[1].type().is_nucleon() ||
