...
void ScatterAction::add_all_processes(double elastic_parameter,
   ...
   * particles are generated by string fragmentation. If in between, the out going
   ...
   * threshold to (mix_scatter_type_energy - mix_scatter_type_window_width), while
...
void ScatterAction::string_excitation_inter() {
  ...
  for (int i = 0; i < npart; i++){
    ...
    log.debug("4-momentum from Pythia: ", momentum);
    const double suppression_factor = 0.7;
    new_particle.set_cross_section_scaling_factor(suppression_factor*spmerge->final_tform[1].at(i));
    new_particle.set_formation_time(spmerge->final_tform[0].at(i)*gamma_cm() +
                                    time_of_execution_);
