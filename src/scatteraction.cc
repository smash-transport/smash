  ...
  // Disable doubleing point exception trap for Pythia
  {
    ...
    while (!final_state_success) {
      ...
      if (final_state_success) {
        FourVector final_mom(0.0, 0.0, 0.0, 0.0);
        for (int i = 0; i < event.size(); i++) {
          if (event[i].isFinal() && event[i].isHadron()) {
            final_mom += FourVector(event[i].e(), event[i].px(),
                                    event[i].py(), event[i].pz());
          }
        }
        const double tolerance = 1e-8 * sqrts;
        if (std::abs(final_mom.x0() - sqrts) > tolerance ||
            std::abs(final_mom.x1()) > tolerance ||
            std::abs(final_mom.x2()) > tolerance ||
            std::abs(final_mom.x3()) > tolerance) {
