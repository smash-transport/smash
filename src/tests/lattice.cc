...
TEST(neighbor_indeces) {
  ...
  RectangularLattice<double> l (
				lattice_l, cell_n, origin, periodicity, LatticeUpdate::EveryTimestep);
  ...
  double ix1_l = ( (r1.x1() - l.cell_sizes()[0]) < l.origin()[0] ) ?
    ix1 : (ix1 - 1);
  double ix1_r = ( (r1.x1() + l.cell_sizes()[0]) >= (l.origin()[0] + l.lattice_sizes()[0]) ) ?
    ix1 : (ix1 + 1);
  double iy1_d = ( (r1.x2() - l.cell_sizes()[1]) < l.origin()[1] ) ?
    iy1 : (iy1 - 1);
  double iy1_u = ( (r1.x2() + l.cell_sizes()[1]) >= (l.origin()[1] + l.lattice_sizes()[1]) ) ?
    iy1 : (iy1 + 1);
  double iz1_n = ( (r1.x3() - l.cell_sizes()[2]) < l.origin()[2] ) ?
    iz1 : (iz1 - 1);
  double iz1_f = ( (r1.x3() + l.cell_sizes()[2]) >= (l.origin()[2] + l.lattice_sizes()[2]) ) ?
    iz1 : (iz1 + 1);
  // compare
  COMPARE(l.cell_center( ix1_l, iy1, iz1 ), l.cell_center( l.index_left(ix1, iy1, iz1) ));
  COMPARE(l.cell_center( ix1_r, iy1, iz1 ), l.cell_center( l.index_right(ix1, iy1, iz1) ));
  COMPARE(l.cell_center( ix1, iy1_d, iz1 ), l.cell_center( l.index_down(ix1, iy1, iz1) ));
  COMPARE(l.cell_center( ix1, iy1_u, iz1 ), l.cell_center( l.index_up(ix1, iy1, iz1) ));
  COMPARE(l.cell_center( ix1, iy1, iz1_n ), l.cell_center( l.index_near(ix1, iy1, iz1) ));
  COMPARE(l.cell_center( ix1, iy1, iz1_f ), l.cell_center( l.index_far(ix1, iy1, iz1) ));
  ...
  // get shifted coordinates in the case of a non-periodic lattice
  double ix_l = ( (r2.x1() - l.cell_sizes()[0]) < l.origin()[0] ) ?
    ix2 : (ix2 - 1);
  double ix_r = ( (r2.x1() + l.cell_sizes()[0]) >= (l.origin()[0] + l.lattice_sizes()[0]) ) ?
    ix2 : (ix2 + 1);
  double iy_d = ( (r2.x2() - l.cell_sizes()[1]) < l.origin()[1] ) ?
    iy2 : (iy2 - 1);
  double iy_u = ( (r2.x2() + l.cell_sizes()[1]) >= (l.origin()[1] + l.lattice_sizes()[1]) ) ?
    iy2 : (iy2 + 1);
  double iz_n = ( (r2.x3() - l.cell_sizes()[2]) < l.origin()[2] ) ?
    iz2 : (iz2 - 1);
  double iz_f = ( (r2.x3() + l.cell_sizes()[2]) >= (l.origin()[2] + l.lattice_sizes()[2]) ) ?
    iz2 : (iz2 + 1);
  // compare
  COMPARE( l.cell_center( ix_l, iy2, iz2 ), l.cell_center( l.index_left(ix2, iy2, iz2) ) );
  COMPARE( l.cell_center( ix_r, iy2, iz2 ), l.cell_center( l.index_right(ix2, iy2, iz2) ) );
  COMPARE( l.cell_center( ix2, iy_d, iz2 ), l.cell_center( l.index_down(ix2, iy2, iz2) ) );
  COMPARE( l.cell_center( ix2, iy_u, iz2 ), l.cell_center( l.index_up(ix2, iy2, iz2) ) );
  COMPARE( l.cell_center( ix2, iy2, iz_n ), l.cell_center( l.index_near(ix2, iy2, iz2) ) );
  COMPARE( l.cell_center( ix2, iy2, iz_f ), l.cell_center( l.index_far(ix2, iy2, iz2) ) );
}
...
TEST(neighbor_indeces_periodic) {
  ...
  double ix_l = ( (r.x1() - dx) < l.origin()[0] ) ?
    (ix + l.n_cells()[0] - 1 ) : (ix - 1);
  double ix_r = ( (r.x1() + dx) >= l.lattice_sizes()[0] ) ?
    (ix - l.n_cells()[0] + 1 ) : (ix + 1);
  double iy_d = ( (r.x2() - dy) < l.origin()[1] ) ?
    (iy + l.n_cells()[1] - 1 ) : (iy - 1);
  double iy_u = ( (r.x2() + dy) >= l.lattice_sizes()[1] ) ?
    (iy - l.n_cells()[1] + 1 ) : (iy + 1);
  double iz_n = ( (r.x3() - dz) < l.origin()[2] ) ?
    (iz + l.n_cells()[2] - 1 ) : (iz - 1);
...
TEST(gradient) {
  ...
  RectangularLattice<ThreeVector> grad_lat (
					    l, n, origin, periodicity, LatticeUpdate::EveryTimestep);
  ...
  */
  grad_lat.iterate_sublattice({0, 0, 0}, grad_lat.n_cells(),
...
TEST(compute_four_gradient_lattice) {
  ...
  const std::array<double, 3> origin = {-5.2, -4.3, -6.7};
  bool periodicity = false;
  RectangularLattice<FourVector> old_lat (
					  l, n, origin, periodicity, LatticeUpdate::EveryTimestep);
  RectangularLattice<FourVector> new_lat (
					  l, n, origin, periodicity, LatticeUpdate::EveryTimestep);
  ...
  new_lat.iterate_sublattice({0, 0, 0}, new_lat.n_cells(),
     [&](FourVector &node, int ix, int iy, int iz){
       ...
       // filling each element of the four-vector at the node with the same function
  ...
  */
  grad_lat.iterate_sublattice({0, 0, 0}, grad_lat.n_cells(),
	 ...
	 "node: (" << ix << ", " << iy << ", " << iz << ")";
       ...
       expected_grad.set_x1(-2*M_PI/l[0] * std::sin(2*M_PI*pos.x1()/l[0]) *
			    std::cos(2*M_PI*pos.x2()/l[1]) *
			    std::cos(2*M_PI*pos.x3()/l[2]));
       expected_grad.set_x2(-2*M_PI/l[1] * std::cos(2*M_PI*pos.x1()/l[0]) *
			    std::sin(2*M_PI*pos.x2()/l[1]) *
			    std::cos(2*M_PI*pos.x3()/l[2]));
       expected_grad.set_x3(-2*M_PI/l[2] * std::cos(2*M_PI*pos.x1()/l[0]) *
			    std::cos(2*M_PI*pos.x2()/l[1]) *
...
TEST(iterate_in_rectangle) {
  ...
  /* Iterate all the lattice and check that marked nodes are within the rectangle,
     while not marked nodes are out of the rectangle */
  lattice->iterate_sublattice(
      {0, 0, 0}, lattice->n_cells(),
      [&](FourVector &node, int ix, int iy, int iz) {
        r = lattice->cell_center(ix, iy, iz);
        if (std::abs(r[0] - r0[0]) <= rectangle[0] &&
          ...
          COMPARE(node, mark) << ix << " " << iy << " " << iz << "\t periodic = false";
        } else {
          COMPARE(node, FourVector()) << ix << " " << iy << " " << iz << "\t periodic = false";
        }
      });
  ...
  lattice->iterate_sublattice(
	  ...
	  d[i] = (d1[i] < d2[i]) ? d1[i] : d2[i];
        ...
        if (d[0] <= rectangle[0] && d[1] <= rectangle[1] && d[2] <= rectangle[2]) {
          COMPARE(node, mark) << ix << " " << iy << " " << iz << "\t periodic = true"
...
TEST(iterate_nearest_neighbors) {
  ...
  int current_index = 0;
  // Iterate all the lattice and check that the center node and the neighboring nodes
  // are marked
  lattice->iterate_sublattice(
      {0, 0, 0}, lattice->n_cells(),
      [&](FourVector &node, int ix, int iy, int iz) {
