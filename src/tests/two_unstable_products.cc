...
TEST(pp_DeltaDelta_integral) {
  ...
  COMPARE_ABSOLUTE_ERROR(delta_plus.spectral_function_no_norm(1.3), 2.343254132, 1.e-9);
  COMPARE_ABSOLUTE_ERROR(delta_plus.spectral_function(1.3), 2.044684068, 1.e-9);
  std::vector<double> sqrts_list = {2.662, 3.162, 3.662, 4.162, 4.662, 5.162, 5.662, 50.};
  std::vector<double> integrals2d = {0.180796, 0.539887, 0.858989,
                                     1.15759, 1.44408, 1.72266, 1.99572, 24.3628};
  ...
  /* Integrals were independently computed with the following Mathematica code:
     ...
     G[m_] := GD0* mD0/m*(pCM[m, mN, mpi]/
       pCM[mD0, mN, mpi])^3*(pCM[mD0, mN, mpi]^2 + La^2)/(pCM[m, mN, mpi]^2 + La^2)
...
TEST(f2_width) {
  ...
  std::vector<double> f2mass_list = {0.862, 1.162, 1.462, 1.762, 2.062,
                              2.362, 2.662, 2.962, 3.262, 3.562, 3.862,
                              4.162, 4.462, 4.762, 5.062, 5.362, 5.662};
  std::vector<double> f2_width_list = {0.00193368, 0.158777, 0.223674, 0.155464,
              0.0480971, 0.0156635, 0.00577278, 0.00237419, 0.00106874,
              0.000518157, 0.000267202, 0.000145138, 0.0000824081, 0.000048615,
              0.0000296518, 0.0000186238, 0.0000120054};
