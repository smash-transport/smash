  ...
  while (!finished) {
    ...
    if (std::get<2>(max_coords) < 0.02) {
      ...
      std::cout << "ds= " << ds << ", dt= " << dt
                ...
                << std::get<0>(max_coords) << " " << std::get<1>(max_coords) << std::endl;
      dt *= 0.75;
      ds *= 0.75;
    }
  }
...
TEST(pi_rho_pi0) {
  ...
  fs.open(
      "/home/jonas/Master/cross_sections_tests/diff_xs/pi_rho_pi0/"
      "dt_005_ds_01.dat",
      std::fstream::out);
...
TEST(sin) {
  ...
  std::fstream fs;
  fs.open("/home/jonas/Master/cross_sections_tests/diff_xs/easy_functions/xxyy.dat", std::fstream::out);
  double x, y;
  double x_poll = 0.1 * dx, y_poll = 0.1 * dy;
