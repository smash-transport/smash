...
TEST(validators_collider) {
  ...
  VERIFY(targ_diff.validate(0.5));
  VERIFY(!targ_diff.validate(0.0));
  VERIFY(!targ_diff.validate(1.0));
  const auto& projectile_particles =
      InputKeys::modi_collider_projectile_particles;
  VERIFY(projectile_particles.validate(std::map<PdgCode, int>{{0x2212, 42}}));
  VERIFY(!projectile_particles.validate(std::map<PdgCode, int>{{0x2212, -2}}));
  ...
  VERIFY(targ_sat.validate(0.175));
