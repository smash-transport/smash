...
void Lorentz::Boost2_RestToLab(int dim, double *U, double **Tin, double **Tout){
	int i,ii,j,jj;
	//double **Lambda;
	Lambda = static_cast<double **>( malloc((dim+1)*sizeof(double *)) );
	for(i=0;i<=dim;i++){
...
void Lorentz::Boost2_LabToRest(int dim, double *U, double **Tin, double **Tout){
	int i,ii,j,jj;
	//double **Lambda;
	Lambda = static_cast<double **>( malloc((dim+1)*sizeof(double *)) );
	for(i=0;i<=dim;i++){
...
int SPmerge::append_finalArray(double *uString, double *evecLong){
	...
	nfrag = 0;
	bstring = 0;
	for(ipyth = 0; ipyth < pythia->event.size(); ipyth++){ if(pythia->event[ipyth].isFinal()) {
		nfrag = nfrag + 1;
	...
	for(ipyth = 0; ipyth < pythia->event.size(); ipyth++){ if(pythia->event[ipyth].isFinal()) {
		...
		if( evecLong == NULL ){
			...
			pparallel[ipstr]
				= pxfrag[ipstr]*evecLong[1] + pyfrag[ipstr]*evecLong[2] + pzfrag[ipstr]*evecLong[3];
			//fprintf(stderr,"  SPmerge::append_finalArray : non-zero evecLong\n");
		}
		Yparallel[ipstr] = 0.5*log( (Efrag[ipstr] + pparallel[ipstr])/(Efrag[ipstr] - pparallel[ipstr]) );
	...
	for(kpstr = 0;kpstr < nfrag; kpstr++){
		...
		XVertexPos[kpstr + 1] = XVertexPos[kpstr]
	...
	for(kpstr = nfrag - 1; kpstr >= 0; kpstr--){
		...
		XVertexNeg[kpstr] = XVertexNeg[kpstr + 1]
			- (Efrag[ipstr] - pparallel[ipstr])/(kappaString*sqrt(2.));
	}
	...
	for(kpstr = 0;kpstr < nfrag; kpstr++){
		...
		if( evecLong == NULL ){
			...
			xfRS[1] = evecLong[1]*(XVertexPos[kpstr] - XVertexNeg[kpstr + 1])/sqrt(2.);
			xfRS[2] = evecLong[2]*(XVertexPos[kpstr] - XVertexNeg[kpstr + 1])/sqrt(2.);
			xfRS[3] = evecLong[3]*(XVertexPos[kpstr] - XVertexNeg[kpstr + 1])/sqrt(2.);
		}
		...
		if( abs(bstring) == 0 ){ // mesonic string
			if( ( kpstr == 0 ) && ( foundFW == false ) ){
				...
				if( abs(baryon) == 3 ){
					...
					fprintf(stderr,"  SPmerge::append_finalArray warning : particle is not meson or baryon.\n");
				}
				foundFW = true;
			}
			...
			else if( ( kpstr == (nfrag - 1) ) && ( baryon == 0 )
				&& ( foundFW == true ) && ( foundBW == false ) ){
				islead = 1;
				xtotfac = 0.5;
				foundBW = true;
			}
			...
			fprintf(stderr,"  SPmerge::append_finalArray warning : string is neither mesonic nor baryonic.\n");
			islead = 0;
			xtotfac = 0.;
		}
...
bool SPmerge::check_conservation(){
	...
	for(ipart = 0; ipart < npart; ipart++){
		...
		BFIN = BFIN + pythia->particleData.baryonNumberType( final_PDGid[0].at(ipart) );
	...
	if( CFIN != CINI ){
		fprintf(stderr,"  SPmerge::check_conservation : charge violation - CINI = %d, CFIN = %d\n",
			CINI, CFIN);
		ret = false;
	}

	if( BFIN != BINI ){
		fprintf(stderr,"  SPmerge::check_conservation : baryon number violation - BINI = %d, BFIN = %d\n",
			BINI, BFIN);
		ret = false;
	}

	if( fabs( EFIN - EINI ) > 1.0e-8*EINI ){
		fprintf(stderr,"  SPmerge::check_conservation : energy violation - EINI = %e GeV, EFIN = %e GeV\n",
			EINI, EFIN);
		ret = false;
	}

	if( fabs( pxFIN - pxINI ) > 1.0e-8*EINI ){
		fprintf(stderr,"  SPmerge::check_conservation : momentum x violation - pxINI = %e GeV, pxFIN = %e GeV\n",
			pxINI, pxFIN);
		ret = false;
	}

	if( fabs( pyFIN - pyINI ) > 1.0e-8*EINI ){
		fprintf(stderr,"  SPmerge::check_conservation : momentum y violation - pyINI = %e GeV, pyFIN = %e GeV\n",
			pyINI, pyFIN);
		ret = false;
	}

	if( fabs( pzFIN - pzINI ) > 1.0e-8*EINI ){
		fprintf(stderr,"  SPmerge::check_conservation : momentum z violation - pzINI = %e GeV, pzFIN = %e GeV\n",
			pzINI, pzFIN);
		ret = false;
	}
...
bool SPmerge::init_lab(int idAIn, int idBIn, double massAIn, double massBIn, Vec4 plabAIn, Vec4 plabBIn){
	...
	py = plabA[2] + plabB[2];
	pz = plabA[3] + plabB[3];
	sqrtsAB = sqrt( pow(fabs(E),2.) - pow(fabs(px),2.) - pow(fabs(py),2.) - pow(fabs(pz),2.) );
...
bool SPmerge::next_SDiff_AX(){
	...
	if( mstrMin > mstrMax ){
		...
		fprintf(stderr,"  SPmerge::next_SDiff_AX : mstrMin = %e GeV, mstrMax = %e GeV\n",
			mstrMin, mstrMax);
		ntry = 100;
	}
	while( ( ( foundPabsX == false ) || ( foundMassX == false ) ) && ( ntry < 100 ) ){
		...
		pabscomAX = sqrt( ( pow(fabs(sqrtsAB),2.) - pow(fabs(massA + massX),2.)
			)*( pow(fabs(sqrtsAB),2.) - pow(fabs(massA - massX),2.) ) )/(2.*sqrtsAB);

		foundPabsX = pabscomAX > QTrn;
		foundMassX = massX > ( pythia->particleData.m0(idqX1) + pythia->particleData.m0(idqX2) );
	}
	...
	if( ( foundPabsX == true ) && ( foundMassX == true ) ){
		...
		for(imu = 1; imu < 4; imu++){
			pstrHcom[imu] = evecBasisAB[3][imu]*sqrt( pow(fabs(pabscomAX),2.) - pow(fabs(QTrn),2.) )
				+ evecBasisAB[1][imu]*QTrn*cos(phiQ)
				+ evecBasisAB[2][imu]*QTrn*sin(phiQ);
			pstrXcom[imu] = -evecBasisAB[3][imu]*sqrt( pow(fabs(pabscomAX),2.) - pow(fabs(QTrn),2.) )
				- evecBasisAB[1][imu]*QTrn*cos(phiQ)
				- evecBasisAB[2][imu]*QTrn*sin(phiQ);
		}
		//fprintf(stderr,"  SPmerge::next_SDiff_AX : pstrXcom = (%e, %e, %e, %e) GeV\n",
		...
		pnull[0] = sqrt( pow(fabs(pnull[1]),2.) + pow(fabs(pnull[2]),2.) + pow(fabs(pnull[3]),2.) );
		lorentz->Boost1_LabToRest(3, ustrXcom, pnull, prs);
		pabs = sqrt( pow(fabs(prs[1]),2.) + pow(fabs(prs[2]),2.) + pow(fabs(prs[3]),2.) );
		evec[0] = 0.;
		for(imu = 1; imu < 4; imu++){
...
bool SPmerge::next_SDiff_XB(){
	...
	if( mstrMin > mstrMax ){
		...
		fprintf(stderr,"  SPmerge::next_SDiff_XB : mstrMin = %e GeV, mstrMax = %e GeV\n",
			mstrMin, mstrMax);
		ntry = 100;
	}
	while( ( ( foundPabsX == false ) || ( foundMassX == false ) ) && ( ntry < 100 ) ){
		...
		pabscomXB = sqrt( ( pow(fabs(sqrtsAB),2.) - pow(fabs(massB + massX),2.)
			)*( pow(fabs(sqrtsAB),2.) - pow(fabs(massB - massX),2.) ) )/(2.*sqrtsAB);
	...
	if( ( foundPabsX == true ) && ( foundMassX == true ) ){
		...
		for(imu = 1; imu < 4; imu++){
			pstrHcom[imu] = -evecBasisAB[3][imu]*sqrt( pow(fabs(pabscomXB),2.) - pow(fabs(QTrn),2.) )
				- evecBasisAB[1][imu]*QTrn*cos(phiQ)
				- evecBasisAB[2][imu]*QTrn*sin(phiQ);
			pstrXcom[imu] = evecBasisAB[3][imu]*sqrt( pow(fabs(pabscomXB),2.) - pow(fabs(QTrn),2.) )
				+ evecBasisAB[1][imu]*QTrn*cos(phiQ)
				+ evecBasisAB[2][imu]*QTrn*sin(phiQ);
		}
		//fprintf(stderr,"  SPmerge::next_SDiff_XB : pstrXcom = (%e, %e, %e, %e) GeV\n",
		//	pstrXcom[0], pstrXcom[1], pstrXcom[2], pstrXcom[3]);
...
bool SPmerge::next_DDiff_XX(){
	...
	//PDGid2idqset(PDGidB, idqsetB);
	PPosA = ( pcomA[0]
		+ evecBasisAB[3][1]*pcomA[1] + evecBasisAB[3][2]*pcomA[2] + evecBasisAB[3][3]*pcomA[3] )/sqrt(2.);
	PNegA = ( pcomA[0]
		- evecBasisAB[3][1]*pcomA[1] - evecBasisAB[3][2]*pcomA[2] - evecBasisAB[3][3]*pcomA[3] )/sqrt(2.);
	PPosB = ( pcomB[0]
		+ evecBasisAB[3][1]*pcomB[1] + evecBasisAB[3][2]*pcomB[2] + evecBasisAB[3][3]*pcomB[3] )/sqrt(2.);
	PNegB = ( pcomB[0]
		- evecBasisAB[3][1]*pcomB[1] - evecBasisAB[3][2]*pcomB[2] - evecBasisAB[3][3]*pcomB[3] )/sqrt(2.);
	...
	while( ( ( foundMass1 == false ) || ( foundMass2 == false ) ) && ( ntry < 100 ) ){
		...
		for(imu = 1; imu < 4; imu++){
			pstr1com[imu] = evecBasisAB[3][imu]*( PPosA + QPos - PNegA - QNeg )/sqrt(2.)
				+ evecBasisAB[1][imu]*QTrn*cos(phiQ)
				+ evecBasisAB[2][imu]*QTrn*sin(phiQ);
			pstr2com[imu] = evecBasisAB[3][imu]*( PPosB - QPos - PNegB + QNeg )/sqrt(2.)
				- evecBasisAB[1][imu]*QTrn*cos(phiQ)
				- evecBasisAB[2][imu]*QTrn*sin(phiQ);
			mstr1 = mstr1 - pow(fabs(pstr1com[imu]),2.);
			mstr2 = mstr2 - pow(fabs(pstr2com[imu]),2.);
		}
		//fprintf(stderr,"  SPmerge::next_DDiff : pstr1com = (%e, %e, %e, %e) GeV\n",
		//	pstr1com[0], pstr1com[1], pstr1com[2], pstr1com[3]);
		//fprintf(stderr,"  SPmerge::next_DDiff : pstr2com = (%e, %e, %e, %e) GeV\n",
		...
		foundMass1 = mstr1 > ( pythia->particleData.m0(idq11) + pythia->particleData.m0(idq12) );
		foundMass2 = mstr2 > ( pythia->particleData.m0(idq21) + pythia->particleData.m0(idq22) );
	}
...
bool SPmerge::next_NDiff(){
	...
	while( ( ( foundMass1 == false ) || ( foundMass2 == false ) ) && ( ntry < 100 ) ){
		...
		if( ( baryonA == 3 ) && ( baryonB == 3 ) ){ // baryon-baryon
			...
			fprintf(stderr,"  SPmerge::next_NDiff : incorrect baryon number of incoming hadrons.\n");
		...
		mstr2 = pow(fabs(pstr2com[0]),2.);
		for(imu = 1; imu < 4; imu++){
			pstr1com[imu] = evecBasisAB[3][imu]*( PPosA + dPPos - PNegA - dPNeg )/sqrt(2.)
				- evecBasisAB[1][imu]*QTrn*cos(phiQ)
				- evecBasisAB[2][imu]*QTrn*sin(phiQ);
			pstr2com[imu] = evecBasisAB[3][imu]*( PPosB - dPPos - PNegB + dPNeg )/sqrt(2.)
				+ evecBasisAB[1][imu]*QTrn*cos(phiQ)
				+ evecBasisAB[2][imu]*QTrn*sin(phiQ);
			mstr1 = mstr1 - pow(fabs(pstr1com[imu]),2.);
			mstr2 = mstr2 - pow(fabs(pstr2com[imu]),2.);
		}
		//fprintf(stderr,"  SPmerge::next_NDiff : pstr1com = (%e, %e, %e, %e) GeV\n",
		//	pstr1com[0], pstr1com[1], pstr1com[2], pstr1com[3]);
		//fprintf(stderr,"  SPmerge::next_NDiff : pstr2com = (%e, %e, %e, %e) GeV\n",
		//	pstr2com[0], pstr2com[1], pstr2com[2], pstr2com[3]);
...
bool SPmerge::next_BBarAnn(){
	...
	if( (baryonA == 3) && (baryonB == -3) ){ // if it is
		...
		mstringMin = pythia->particleData.m0(idq11) + pythia->particleData.m0(idq12);
		Mstring1 = 0.5*sqrtsAB;

		if(Mstring1 < mstringMin){
			fprintf(stderr,"  SPmerge::next_BBarAnn failure : Mstring1 < mstringMin.\n");
			ret = false;
		}

		mstringMin = pythia->particleData.m0(idq21) + pythia->particleData.m0(idq22);
		Mstring2 = 0.5*sqrtsAB;

		if(Mstring2 < mstringMin){
			fprintf(stderr,"  SPmerge::next_BBarAnn failure : Mstring2 < mstringMin.\n");
			ret = false;
		}

		if(ret == true){
			NpartString1 = fragmentString(idq11, idq12, Mstring1, NULL, ranrot);
			NpartString2 = fragmentString(idq21, idq22, Mstring2, NULL, ranrot);
		}
...
int SPmerge::fragmentString(int idq1, int idq2, double mString, double *evecLong, bool ranrot){
	int ret;
	bool Hforced;
	...
	pCM = sqrt( ( pow(fabs(mString),2.) - pow(fabs(m1 + m2),2.)
		)*( pow(fabs(mString),2.) - pow(fabs(m1 - m2),2.) ) )/(2.*mString);
