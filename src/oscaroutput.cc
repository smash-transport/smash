 ...
 * evolution and printed to a separate output to serve as initial conditions for
 * hybrid models.
 *
 ...
 * Both quark and anti-/di-quark taken from A. It makes sense to distinguish it
 * from AB &rarr; AX, because A and B can be particles of different types, for
 * example, a pion and a proton. It matters then, whether the pion creates a
 * string or the proton.
 *
 ...
 * baryon-antibaryon annihilation. One pair qqbar annihilates immediately and
 * then two strings are formed.
 *
 ...
 * &rarr; 2 QCD process by PYTHIA. Here quarks do not simply form a string. They
 * actually scatter on parton level first.
 *
 * \process_type{47} Failed string process, Soft String NNbar annihilation
 * process can fail by lack of energy. This is a tag we add to avoid mislabeling
 * the events.
 *
 * \page doxypage_output_oscar_particles
 * \n
