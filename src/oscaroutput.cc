 ...
 * \li \key \process_type{0} No previous process yet, particle was created at initialization
 * \li \key \process_type{1} Elastic scattering
 * \li \key \process_type{2} Resonance formation (2 &rarr; 1)
 * \li \key \process_type{3} Inelastic binary scattering (2 &rarr; 2)
 * \li \key \process_type{4} Inelastic multi-particle scattering (2 &rarr; 3)
 * \li \key \process_type{5} Resonance decay
 * \li \key \process_type{6} Box wall crossing (due to periodic boundary conditions)
 * \li \key \process_type{7} Forced thermalization, many particles are replaced by a thermalized ensemble
 * \li \key \process_type{8} Hypersurface crossing, Particles are removed from the evolution and printed to a separate output to serve as initial conditions for hybrid models.
 * \li \key \process_type{9} Bremsstrahlung process: a + b -> a + b + photon
 * \li \key \process_type{10} Inelastic multi-particle meson scattering (3 &rarr; 1)
 * \li \key \process_type{11} Inelastic multi-particle scattering (3 &rarr; 2)
 * \li \key \process_type{12} Inelastic multi-particle scattering (5 &rarr; 2)
 * \li \key \process_type{13} Inelastic multi-particle scattering (2 &rarr; 5)
 * \li \key \process_type{14} Inelastic multi-particle scattering (4 &rarr; 2)
 * \li \key \process_type{15} Inelastic multi-particle scattering (2 &rarr; 4)
 * \li \key \process_type{41} Soft string excitation, single diffractive AB &rarr; AX. Both quark and anti-/di-quark taken from B.
 * \li \key \process_type{42} Soft string excitation, single diffractive AB &rarr; XB
 * \li \key \process_type{43} Soft string excitation, double diffractive. Two strings are formed, one from A and one from B.
 * \li \key \process_type{44} Soft string N-Nbar annihilation
 * \li \key \process_type{45} Soft sring excitation, non-diffractive. Two strings are formed both have ends in A and B.
 * \li \key \process_type{46} Hard string excitation
 * \li \key \process_type{47} Failed string process
 *
 * \page doxypage_output_oscar_particles
 * \n
 * **Event end line**\n
