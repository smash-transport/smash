 ...
 * <tr><td>\key t, \key x, \key y, \key z<td> double <td>Space-time
 coordinates</tr>
 * <tr><td>\key mass<td>double <td>Particle's rest-mass </tr>
 * <tr><td>\key p0, \key px, \key py, \key pz<td> double <td>Energy and
 3-momentum</tr>
 * <tr><td>\key pdg<td>int32_t <td>PDG code of the particle (see
 http://pdg.lbl.gov/). It contains all quantum numbers and uniquely identifies
 its type</tr>
 * <tr><td>\key ID, id<td> int32_t <td>Particle identifier in terms of an
 integer. It is unique for every particle in the event. \key ID is used in the
 OSCAR 2013 standard, while \key id is used in OSCAR 1999</tr>
 * <tr><td>\key charge<td>int32_t <td>Electric charge of the particle in units
 of the elementary charge e</tr>
 * <tr><td>\key ncoll<td>int32_t <td>Number of collisions the particle has
 undergone</tr>
 * <tr><td>\key form_time<td> double <td>Formation time of the particle</tr>
 * <tr><td>\key xsecfac<td> double <td>Cross section scaling factor (if the
 particles are not yet fully formed at the time of interaction, the cross
 section for the underlying process is scaled down by the cross section scaling
 factor)</tr>
 * <tr><td>\key proc_id_origin<td> int32_t <td>ID of the process of the
 particle's last interaction</tr>
 * <tr><td>\key proc_type_origin<td> int32_t <td>Type of the last process the
 particle has undergone. The possible process types are listed in \ref
 doxypage_output_oscar_particles_process_types</tr>
 * <tr><td>\key time_last_coll<td> double <td>Time of the particle's last
 interaction (except wall crossings)</tr>
 * <tr><td>\key pdg_mother1<td> int32_t <td>PDG code of the 1st mother particle
 (0 in case the particle is sampled in a thermal bubble. It is not updated by
 elastic scatterings.)</tr>
 * <tr><td>\key pdg_mother2<td> int32_t <td>PDG code of the 2nd mother particle
 (0 in case the particle results from the decay of a resonance or the appearance
 of a thermal bubble. In the former case, \key pdg_mother1 is the PDG code of
 this resonance. It is not updated by elastic scatterings.)</tr>
 * <tr><td>\key baryon_number<td> int32_t <td>Baryon number of the particle. 1
 for baryons, -1 for anti-baryons and 0 for mesons</tr>
 * <tr><td>\key strangeness<td> int32_t <td>Net-strangeness of the
 particles</tr>
 * <tr><td>\key 0<td> - <td>Prints a column of 0 (for compatibility with OSCAR
 1999)</tr>
 * </table>
