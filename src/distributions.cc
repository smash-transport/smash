 ...
 * ****************************************************************************
 *
 * This is the same function as written here before;
 * however, the distribution as defined here is easier to handle numerically
 * than the one originally implemented  in SMASH. This is because
...
double juttner_distribution_func(double momentum_radial, double mass,
                                 ...
                                 double statistics) {
  return (std::exp(
             -(std::sqrt(momentum_radial * momentum_radial + mass * mass) -
               effective_chemical_potential) /
             temperature)) /
         (1 +
          statistics * std::exp(-(std::sqrt(momentum_radial * momentum_radial +
                                            mass * mass) -
                                  effective_chemical_potential) /
                                temperature));
}
