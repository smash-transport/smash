...
Permutation generate_sort_permutation(
    std::vector<T> const& v, Cmp compare)
{
...
void apply_permutation(
    std::vector<T>& v, const Permutation& p)
{
...
InterpolateData<T>::InterpolateData(const std::vector<T>& x,
  ...
  const auto p = generate_sort_permutation(x,
      [&](T const& a, T const& b) {
        return a < b;
      });
  ...
  for (size_t i = 0; i < n - 1; i++) {
    f_.emplace_back(InterpolateLinear<T>(
        x[i], y_sorted[i], x[i + 1], y_sorted[i + 1]));
  }
}
