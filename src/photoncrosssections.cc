  ...
  const double xs =
      (pow(Const, 2) * pow(g_POR, 4) *
       ((pow(pow(m_omega_, 2) - s, 2) *
         (pow(m_pion_, 8) - 2 * pow(m_pion_, 6) * pow(m_rho, 2) +
          pow(m_pion_, 4) *
              (pow(m_rho, 4) + 4 * pow(m_omega_, 4) - 2 * pow(m_omega_, 2) * s) +
          pow(m_omega_, 4) *
              (pow(m_rho, 4) + pow(m_omega_, 4) + 2 * pow(m_omega_, 2) * s +
               2 * pow(s, 2) - 2 * pow(m_rho, 2) * (pow(m_omega_, 2) + s)) -
          2 * pow(m_pion_, 2) * pow(m_omega_, 2) *
              (pow(m_rho, 4) + 2 * pow(m_omega_, 2) * (pow(m_omega_, 2) + s) -
               pow(m_rho, 2) * (2 * pow(m_omega_, 2) + s)))) /
            (pow(m_omega_, 2) - tmax) +
        (pow(m_pion_, 8) - 2 * pow(m_pion_, 6) * pow(m_rho, 2) + 3 * pow(m_omega_, 8) -
         4 * pow(m_omega_, 6) * s - 7 * pow(m_omega_, 4) * pow(s, 2) +
         4 * pow(m_omega_, 2) * pow(s, 3) + 5 * pow(s, 4) +
         pow(m_rho, 4) *
             (pow(m_omega_, 4) - 2 * pow(m_omega_, 2) * s + 2 * pow(s, 2)) +
         pow(m_rho, 2) *
             (-4 * pow(m_omega_, 6) + 8 * pow(m_omega_, 4) * s - 6 * pow(s, 3)) -
         2 * pow(m_pion_, 2) *
             (4 * pow(m_omega_, 6) -
              2 * pow(m_rho, 2) * pow(pow(m_omega_, 2) - 2 * s, 2) +
              pow(m_rho, 4) * s - 10 * pow(m_omega_, 4) * s + 8 * pow(s, 3)) +
         pow(m_pion_, 4) *
             (pow(m_rho, 4) + 2 * pow(m_rho, 2) * (pow(m_omega_, 2) - s) +
              4 * (pow(m_omega_, 4) - 3 * pow(m_omega_, 2) * s + 3 * pow(s, 2)))) *
            tmax -
        2 * pow(m_pion_, 2) * pow(m_omega_, 4) * pow(tmax, 2) -
        pow(m_rho, 2) * pow(m_omega_, 4) * pow(tmax, 2) +
        pow(m_omega_, 6) * pow(tmax, 2) - pow(m_pion_, 4) * s * pow(tmax, 2) +
        pow(m_pion_, 2) * pow(m_rho, 2) * s * pow(tmax, 2) +
        8 * pow(m_pion_, 2) * pow(m_omega_, 2) * s * pow(tmax, 2) +
        3 * pow(m_rho, 2) * pow(m_omega_, 2) * s * pow(tmax, 2) -
        2 * pow(m_omega_, 4) * s * pow(tmax, 2) -
        8 * pow(m_pion_, 2) * pow(s, 2) * pow(tmax, 2) -
        3 * pow(m_rho, 2) * pow(s, 2) * pow(tmax, 2) -
        3 * pow(m_omega_, 2) * pow(s, 2) * pow(tmax, 2) +
        5 * pow(s, 3) * pow(tmax, 2) +
        ((pow(m_omega_, 4) - 4 * pow(m_omega_, 2) * s + 5 * pow(s, 2)) *
         pow(tmax, 3)) /
            3. -
        (pow(pow(m_omega_, 2) - s, 2) *
         (pow(m_pion_, 8) - 2 * pow(m_pion_, 6) * pow(m_rho, 2) +
        ...
        (pow(m_pion_, 8) - 2 * pow(m_pion_, 6) * pow(m_rho, 2) + 3 * pow(m_omega_, 8) -
         ...
         pow(m_pion_, 4) *
             (pow(m_rho, 4) + 2 * pow(m_rho, 2) * (pow(m_omega_, 2) - s) +
              4 * (pow(m_omega_, 4) - 3 * pow(m_omega_, 2) * s + 3 * pow(s, 2)))) *
            tmin +
        2 * pow(m_pion_, 2) * pow(m_omega_, 4) * pow(tmin, 2) +
        pow(m_rho, 2) * pow(m_omega_, 4) * pow(tmin, 2) -
        pow(m_omega_, 6) * pow(tmin, 2) + pow(m_pion_, 4) * s * pow(tmin, 2) -
        pow(m_pion_, 2) * pow(m_rho, 2) * s * pow(tmin, 2) -
        8 * pow(m_pion_, 2) * pow(m_omega_, 2) * s * pow(tmin, 2) -
        3 * pow(m_rho, 2) * pow(m_omega_, 2) * s * pow(tmin, 2) +
        2 * pow(m_omega_, 4) * s * pow(tmin, 2) +
        8 * pow(m_pion_, 2) * pow(s, 2) * pow(tmin, 2) +
        3 * pow(m_rho, 2) * pow(s, 2) * pow(tmin, 2) +
        3 * pow(m_omega_, 2) * pow(s, 2) * pow(tmin, 2) -
        ...
        2 * (pow(m_omega_, 2) - s) *
            (-pow(m_pion_, 8) +
             pow(m_pion_, 4) * (4 * pow(m_omega_, 4) - 7 * pow(m_omega_, 2) * s +
                              pow(s, 2) + pow(m_rho, 2) * (pow(m_omega_, 2) + s)) +
             pow(m_pion_, 2) *
                 (-6 * pow(m_omega_, 6) + 6 * pow(m_omega_, 4) * s +
                  8 * pow(m_omega_, 2) * pow(s, 2) +
                  pow(m_rho, 4) * (-pow(m_omega_, 2) + s) +
                  pow(m_rho, 2) * (4 * pow(m_omega_, 4) - 7 * pow(m_omega_, 2) * s -
                                  pow(s, 2))) +
             pow(m_omega_, 2) *
                 (2 * pow(m_omega_, 6) + pow(m_rho, 4) * (pow(m_omega_, 2) - s) -
                  4 * pow(m_omega_, 2) * pow(s, 2) - 3 * pow(s, 3) +
                  pow(m_rho, 2) * (-3 * pow(m_omega_, 4) + 2 * pow(m_omega_, 2) * s +
                                  3 * pow(s, 2)))) *
            log((-pow(m_omega_, 2) + tmax) / (-pow(m_omega_, 2) + tmin)))) /
      (128. * Pi * pow(pow(m_omega_, 2) - s, 2) *
       (pow(m_pion_, 4) + pow(pow(m_rho, 2) - s, 2) -
        2 * pow(m_pion_, 2) * (pow(m_rho, 2) + s)));
...
double PhotonCrossSection<ComputationMethod::Analytic>::xs_diff_pi0_rho0_pi0(
  ...
  double diff_xs =
      (pow(Const, 2) * pow(g_POR, 4) *
       (pow(m_omega_, 4) * pow(s, 4) + 4 * pow(m_omega_, 4) * pow(s, 3) * t -
        4 * pow(m_omega_, 2) * pow(s, 4) * t +
        10 * pow(m_omega_, 4) * pow(s, 2) * pow(t, 2) -
        16 * pow(m_omega_, 2) * pow(s, 3) * pow(t, 2) +
        5 * pow(s, 4) * pow(t, 2) + 4 * pow(m_omega_, 4) * s * pow(t, 3) -
        16 * pow(m_omega_, 2) * pow(s, 2) * pow(t, 3) +
        10 * pow(s, 3) * pow(t, 3) + pow(m_omega_, 4) * pow(t, 4) -
        4 * pow(m_omega_, 2) * s * pow(t, 4) + 5 * pow(s, 2) * pow(t, 4) +
        pow(m_pion_, 8) * pow(-2 * pow(m_omega_, 2) + s + t, 2) -
        2 * pow(m_pion_, 6) * pow(m_rho, 2) *
            (2 * pow(m_omega_, 4) + pow(s, 2) + pow(t, 2) -
             2 * pow(m_omega_, 2) * (s + t)) +
        pow(m_rho, 4) *
            (2 * pow(s, 2) * pow(t, 2) - 2 * pow(m_omega_, 2) * s * t * (s + t) +
             pow(m_omega_, 4) * (pow(s, 2) + pow(t, 2))) -
        2 * pow(m_rho, 2) *
            (3 * pow(s, 2) * pow(t, 2) * (s + t) -
             3 * pow(m_omega_, 2) * s * t * pow(s + t, 2) +
             pow(m_omega_, 4) * (pow(s, 3) + 2 * pow(s, 2) * t +
                               2 * s * pow(t, 2) + pow(t, 3))) +
        pow(m_pion_, 4) *
            (-2 * pow(m_rho, 2) * (pow(m_omega_, 2) - s) * (pow(m_omega_, 2) - t) *
                 (s + t) -
             8 * pow(m_omega_, 2) * s * t * (s + t) +
             4 * pow(m_omega_, 4) * (pow(s, 2) + pow(t, 2)) -
             2 * s * t * (pow(s, 2) - 6 * s * t + pow(t, 2)) +
             pow(m_rho, 4) * (2 * pow(m_omega_, 4) + pow(s, 2) + pow(t, 2) -
                             2 * pow(m_omega_, 2) * (s + t))) -
        2 * pow(m_pion_, 2) *
            (2 * (s + t) * pow(-2 * s * t + pow(m_omega_, 2) * (s + t), 2) +
             pow(m_rho, 4) * (-4 * pow(m_omega_, 2) * s * t +
                             pow(m_omega_, 4) * (s + t) + s * t * (s + t)) -
             pow(m_rho, 2) *
                 (-10 * pow(m_omega_, 2) * s * t * (s + t) +
                  2 * pow(m_omega_, 4) * (pow(s, 2) + 3 * s * t + pow(t, 2)) +
                  s * t * (pow(s, 2) + 8 * s * t + pow(t, 2)))))) /
      (128. * Pi * pow(pow(m_omega_, 2) - s, 2) *
       (pow(pow(m_pion_, 2) - pow(m_rho, 2), 2) -
        2 * (pow(m_pion_, 2) + pow(m_rho, 2)) * s + pow(s, 2)) *
