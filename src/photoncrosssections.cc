...
double PhotonCrossSection<ComputationMethod::Analytic>::xs_pi0_rho_pi(
  ...
  const double xs =
          to_mb*((pow(Const,2)*pow(ghat,4)*(0. - (0.25*pow(-2 + delta,2)*pow(mpion,2)*
...
double PhotonCrossSection<ComputationMethod::Analytic>::xs_pi_pi_rho0(
  ...
  const double xs =
          to_mb*(-(pow(Const,2)*pow(ghat,4)*(0. + (0.03125*pow(eta1 -
							1.*eta2,2)*(eta1*eta2*
...
double PhotonCrossSection<ComputationMethod::Analytic>::xs_diff_pi0_rho0_pi0(
  ...
  const double mpion = m_pion_, &mrho = m_rho;
  double diff_xs =
   (pow(Const,2)*pow(g_POR,4)*(pow(momega,4)*pow(s,4) + 4*pow(momega,4)*pow(s,3)*t - 4*pow(momega,2)*pow(s,4)*t + 10*pow(momega,4)*pow(s,2)*pow(t,2) -
   ...
   (3.0*128.*Pi*pow(pow(momega,2) - s,2)*(pow(pow(mpion,2) - pow(mrho,2),2) - 2*(pow(mpion,2) + pow(mrho,2))*s + pow(s,2))*pow(pow(momega,2) - t,2));



	return to_mb * diff_xs;
}
...
double PhotonCrossSection<ComputationMethod::Analytic>::xs_diff_pi_pi_rho0(
  ...
  const double diff_xs =
      ((pow(Const,2)*pow(ghat,4)*((0.25*(32*pow(C4,2)*pow(mrho,8) +
							2*pow(delta,2)*pow(s,2) + 8*C4*pow(mrho,6)*(-6 + delta - 8*C4*s) +
							           2*delta*pow(mrho,2)*s*(-6 + delta - 8*C4*s) +
							pow(mrho,4)*(12 - pow(delta,2) + 8*C4*(6 + delta)*s +
...
double PhotonCrossSection<ComputationMethod::Analytic>::xs_diff_pi_pi0_rho(
  ...
  const double mpion = m_pion_, mrho = m_rho;
  const double diff_xs =
     (pow(Const,2)*pow(ghat,4)*((-0.25*pow(-2 + delta,2)*pow(mpion,2)*
...
double PhotonCrossSection<ComputationMethod::Analytic>::xs_diff_pi_rho0_pi(
  ...
  const double diff_xs =
	...
	(pow(Const,2)*pow(ghat,4)*((-8*pow(-2 + delta,2)*pow(mpion,2))/(pow(mrho,2)*pow(pow(mpion,2) - s,2)) -
            ...
            (eta2*(-2*pow(mpion,4)*(delta - 4*C4*pow(mrho,2))*(pow(mrho,2) + 4*t) +
               ...
               eta1*(2*delta*t*pow(s + t,2) - pow(mrho,2)*((-2 + delta)*pow(s,2) + 2*(-2 + 3*delta)*s*t + (-6 + 5*delta)*pow(t,2) + 8*C4*t*pow(s + t,2)) +
                  ...
                  2*pow(mpion,2)*(4*delta*t*(s + t) - pow(mrho,2)*(-2*s + 3*delta*s - 6*t + 7*delta*t + 16*C4*t*(s + t)) +
                     2*pow(mrho,4)*(-2 + delta + 4*C4*(s + 2*t)))))/(pow(mrho,2)*(-pow(ma1,2) + t))))/
        (pow(mpion,4) + pow(pow(mrho,2) - s,2) - 2*pow(mpion,2)*(pow(mrho,2) + s))))/(3.0*512.*Pi);
  ...
  const double diff_xs =
       ...
       (2*((0.0625*(-2. + delta)*(-2.*pow(mrho,2) + delta*(2.*pow(mpion,2) + pow(mrho,2) - 1.*s - 1.*t))*
            ...
            (0.0625*(-2 + delta)*(delta*(6*pow(mpion,6) - pow(mpion,4)*(9*(pow(mrho,2) + s) + t) -
             ...
             (-pow(mpion,2) + s)))/pow(mrho,4)))/(16.*Pi*(pow(mpion,4) + pow(pow(mrho,2) - s,2) - 2*pow(mpion,2)*(pow(mrho,2) + s))))/3.0;
