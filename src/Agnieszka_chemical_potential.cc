...
namespace agnieszka {  
   ...
   double statistics)
  {
    const double distribution_term =
      juttner_distribution_func( (1.0 - x)/x,
				 mass,
				 temperature,
   ...
   double precision)
  {
    ...
    const int error_code = gsl_integration_qags(&Integrand,
						precision,
						1.0,
						epsabs,
						epsrel,
 ...
 /** 
   ...
   gsl_vector * function) 
  {
    struct ParametersForEffectiveChemicalPotentialRootFinderOneSpecies * par =
      static_cast<ParametersForEffectiveChemicalPotentialRootFinderOneSpecies *>
      (parameters);
   ...
   double* effective_chemical_potential)
  {
	...
	if (status == GSL_SUCCESS)
	  {
	    effective_chemical_potential[0] =
	      gsl_vector_get (Root_finder->x, 0);
	    /**
   ...
   double solution_precision)
  {
	...
	throw std::runtime_error
      ...
      const double distribution_at_small_p =
	agnieszka::juttner_distribution_func(small_p,    
					     mass,
					     temperature,
