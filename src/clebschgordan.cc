...
double clebsch_gordan_coefficient(const int j_a, const int j_b, const int j_c,
                                  const int m_a, const int m_b, const int m_c) {
  const clebsch_gordan::ThreeSpins spin_information = {j_a, j_b, j_c,
                                                      m_a, m_b, m_c};
  if (auto search = clebsch_gordan::lookup_table.find(spin_information);
      search != clebsch_gordan::lookup_table.end()) {
    return search->second;
  } else {
    double result = clebsch_gordan_calculation(j_a, j_b, j_c, m_a, m_b, m_c);
    clebsch_gordan::lookup_table.insert({spin_information, result});
    return result;
  }
}
 ...
 * coupling to a total isospin \see clebsch_gordan_coefficient for details
 * (I_tot, I_z). \param[in] p_a Information of particle type for first particle
...
static double isospin_clebsch_gordan_2to1(const ParticleType &p_a,
  ...
  return clebsch_gordan_coefficient(p_a.isospin(), p_b.isospin(), I_tot,
...
double isospin_clebsch_gordan_sqr_3to1(const ParticleType &p_a,
  ...
  const double cg =
      clebsch_gordan_coefficient(I_ab, p_c.isospin(), Res.isospin(), I_abz,
                                 p_c.isospin3(), Res.isospin3()) *
      clebsch_gordan_coefficient(p_a.isospin(), p_b.isospin(), I_ab,
